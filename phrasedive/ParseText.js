...
class ParseText {
    ...
    initialize(file) {
        ...
        console.log('vocabulary size', this.wordArray.length, 'sentences', result.length, 'sentenceLength', this.maxSentenceLength)
    }
    ...
    writeData() {
        fs.writeFile(this.file+".sentenceWordIndex", JSON.stringify(this.getSentenceWordIndex()), function (err) {
        });
        
        fs.writeFile(this.file+".sentence", JSON.stringify(this.getSentence()), function (err) {
        });
        
        fs.writeFile(this.file+".sentenceTokens", JSON.stringify(this.getSentenceTokens()), function (err) {
        });
        
        fs.writeFile(this.file+".vocabulary", JSON.stringify(this.getVocabulary()), function (err) {
        });
        
        fs.writeFile(this.file+".maxData", JSON.stringify(this.getMaxWordsInSentence()), function (err) {
        });
    }
}
