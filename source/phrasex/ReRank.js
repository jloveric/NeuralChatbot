...
let alignmentRank = function (rSet, searchText) {
    ...
    for (let i = 0; i < rSet.length; i++) {
        ...
        if (phrase == phraseOld) {//There can be 1000's of identical phrases so don't re-calculate the score in this case
            let ans = scoreOld;
            ansList.push({
                result: rSet[i], confidence: ans.score * ans.order * ans.size,
                score: deepcopy(ans)
            })
        } else {
...
let reRank = function (hits, searchText, phraseFrequency) {
    ...
    let bestAns = Helper.objectWithBestValue(bestList, (a, b) => {
        ...
        return a.score.score * a.score.order * a.score.size < b.score.score * b.score.order * b.score.size
