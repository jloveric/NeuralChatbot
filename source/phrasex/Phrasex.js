...
class Phrasex extends PhraseMatcher {
	...
	find(searchText, userData) {
		...
		let np = p.then((body) => {
			...
			for (let i = 0; i < orderedList.length; i++) {
				...
				let align = slotFiller.computeQueryIndex(orderedList[i].score, bestMatch, query)
				...
				for (let i = 0; i < queryIndex.length; i++) {
					if (queryIndex[i].index == -1) {
						let simpleWord = queryIndex[i].word.replace(Helper.nonAlphaNumeric, '').toLowerCase()
						let isStop = Helper.isStopWord(simpleWord)
						if (isStop) { //Mark as a stopword
	...
	getWildcardsAndMatch(phrase, keywords, userData) {
		...
		let np = p.then((resArrayList) => {
			return Promise.resolve(this.getWildcardsAndMatchNoSearch(resArrayList, keywords, userData))
		}).catch((reason) => {
			Logger.warn(reason)
 ...
 */
	getWildcardsAndMatchNoSearch(resArrayList, keywords, userData) {
		...
		for (let i = 0; i < resArrayList.length; i++) {
			...
			let wcAndScore = slotFiller.computeWildcards(bestMatch, query,
				queryIndex, keywords)
