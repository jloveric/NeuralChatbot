...
class PhraseDatabase {
	...
	initialize(options) {
		...
		let np = this.mongod.initialize("phrasedb", this.gf.mongodb.url).then((db) => {
			this.db = db;
	...
	getList(phraseType, implies) {
		...
		let np = new Promise((resolve, reject) => {
			...
			table.find({ phraseType: phraseType, implies: implies.sort() }).toArray((err, documents) => {
				if (err) {
					Logger.error(err);
					reject();
				} else {
	...
	addGroup(obj, definitions) {
		...
		let pList = [];
		if (obj.phrase) {//If it has a phrase block
