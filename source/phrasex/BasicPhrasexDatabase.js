...
module.exports = {
	...
	expandFiles(filename) {
		...
		try {
			...
			if (include) {//include the file in the object - non recursive.
	...
	tQ(a, index, definitions, pdb) {
		...
		if (index < a.length) {
			return this.subSet(a, index, definitions, pdb).then(() => {
	...
	generatePhraseDatabase: function (filename, tableName, fileAsText) {
		...
		return pdb.initialize({ phraseTable: tableName }).then(() => {
