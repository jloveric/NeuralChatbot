...
class BasicBot extends SingleResponseIfc {
    ...
    initialize(confShallow) {
        ...
        let p2 = this.pdb.initialize(conf).then(() => {
    ...
    storeData(storage, wildcards, userData, intent, source, botStorage) {
        ...
        for (let i in storage.set) {
            ...
            if (ia[0] == 'other') {
                ...
                userData.storage.insertElement(wildcards, i, storage.set[i], intent, source)
            } else {
                userData.storage.insertElement(wildcards, i, storage.set[i], intent, source)
                botStorage.insertElement(wildcards, i, storage.set[i], intent, source)
            }
    ...
    retrieveData(storage, wildcards, userData, intent, botStorage) {
        ...
        for (let i in storage.get) {
            ...
            if (path[0] == 'other') {
                ...
                let val = botStorage.flattenedObject(i, elem, completeArray.val, wildcards)
                ...
                let val = userData.storage.flattenedObject(i, elem, completeArray.val, wildcards)
     ...
     */
    processResult(resArray, originalUserData, botStorage, ignoreWildcardHistory, scoreBasedOnSearch) {
        ...
        for (let i = 0; i < resArray.length; i++) {
            ...
            debug('------------------------storage-------------------', storage, source.implies[0])
            let info = null;
            if (storage) {
                this.storeData(storage, wildcards, userData, source.implies[0], source, lStorage)

                //You can also fill in the values
                info = this.retrieveData(storage, wildcards, userData, source.implies[0], source, lStorage)
            }
            ...
            debug('wildcards', wildcards, 'history', userData.history, 'lastHistor', userData.getLastHistory())
            if (!ignoreWildcardHistory) {//You ignore the history if your have expanded a word, in general
        ...
        return Promise.all(pList).then((ans) => {
            ...
            debug('FINAL STORAGE-------------------------', originalUserData.storage, final.storage)
            ...
            Helper.logAndThrow('All promises must resolve for this to work!', reason)
    ...
    trimResults(ansObj, infoList) {
        ...
        if (newList.length == 1) {
            return { val: newList[0], userData: newUserData[0], storage: newStorage[0] };
        }
        //debug('ans',ans)
        ...
        if (bestSlotList.length == 1) {
            return { val: bestSlotList[0], userData: bestSlotUserData[0], storage: bestSlotStorage[0] };
        }
        ...
        for(let i=0; i<bestSlotList.length; i++) {
            ...
            if(bestSlotList[i].wcScore) {
                wcScore = bestSlotList[i].wcScore.score ? bestSlotList[i].wcScore.score : 0
            }
        ...
        return { val: final[0], userData: finalUserData[0], storage: finalStorage[0] }
    }
    ...
    standardResult(phrase, userData, explicit) {
        ...
        return this.phrasex.getWildcardsAndMatch(phrase, this.keywords, userData).then((newRes) => {
    ...
    getResult(phrase, userData, forget, explicit) {
        ...
        let np = this.standardResult(phrase, userData, explicit).then((res) => {
            ...
            if (res.dontRespond) {//Don't modify things further'
            ...
            } else if (res.response == '') {//Reaches this point if '' was entered as the phrase (pressed enter)
                res.response = Helper.selectRandom(Helper.defaultResponse);
            }
