...
class PhrasexBot extends BasicBot {
    ...
    standardResult(phrase, userData, explicit) {
        ...
        let np = new Promise((resolve, reject) => {
            ...
            if (!explicit) {
                ...
                let pWord = this.respondToWord(phrase, pWordUserData, pWordStore).then((ans) => {
                ...
                let pFull = this.phrasex.getWildcardsAndMatch(phrase, this.keywords, pFullUserData).then((newRes) => {
                ...
                Promise.all([pWord, pFull]).then((ans) => {
                ...
                super.standardResult(phrase, userData, explicit).then((ans) => {
                    resolve(ans);
                }).catch((reason) => {
                    resolve(Helper.failResponse)
                    Logger.error(reason);
    ...
    respondToWord(word, userData, botStorage) {
        ...
        let newPhrase = slotFiller.reconstructPhrase(
            Helper.selectRandom(this.doc.default), { word: word })

        Logger.warn('Treating the phrase as a word', word, 'Expanding to', newPhrase)
        let tp = this.phrasex.getWildcardsAndMatch(newPhrase.phrase, this.keywords, userData);
