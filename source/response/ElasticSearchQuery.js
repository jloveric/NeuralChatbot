...
class EsearchResultsQuery extends SingleResponseIfc {
		...
		let np = new Promise((resolve, reject) => {
			...
			this.client.indices.exists({ index: this.indexName },
				(error, exists) => {
					...
					if (error) {
						Logger.error('Check if index exists error', this.indexName, 'error', error);
						reject(error);
					}
					...
					if (exists) {
						...
						Logger.error('The elasticsearch index', this.indexName, 'does not exist')
						reject(error);
					}
	...
	removeBoost(optionsIn) {
		...
		let query =
			{
				"index": this.indexName,
				"size": options.numResults ? options.numResults : this.gc.elasticsearch.boostResults,
	...
	searchFields(searchText, fields, optionsIn) {
		...
		debug('Searching for', searchText, 'in', this.indexName,
			'fields', fields, "num results", this.gc.elasticsearch.numResults)
		...
		let query =
			{
				"index": this.indexName,
				"size": options.numResults ? options.numResults : this.gc.elasticsearch.numResults,
				"searchType": "dfs_query_then_fetch",
				"body": {
					"explain": false,
					"highlight": {
