    ...
    computeResult(input, userData, scoreBasedOnSearch) {
        ...
        for (let i = 0; i < this.actionList.length; i++) {
            if (this.actionList[i].filterInput(input)) {
                let actualAns = this.actionList[i].computeResult(input, userData, scoreBasedOnSearch);
                ...
                let np = new Promise((resolve, reject) => {
                    ...
                    Promise.all([actualAns, basicAns]).then((res) => {
                        ...
                        if ((!res[0].success) && res[1].success && input.wildcards.usedStorage) {
