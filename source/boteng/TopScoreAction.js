...
class TopScoreAction extends Action {
  ...
  computeResult(input, userData) {
    ...
    Helper.hasProperties(input, ['replies',
      'wildcards'])
    ...
    return prom.then((result) => {
      ...
      if (result[0]) {
        let tScore = result[0].score.score * result[0].score.size * result[0].score.order;
        searchConfidence = result[0].score.score ? result[0].score.score : 0.0;
      }
