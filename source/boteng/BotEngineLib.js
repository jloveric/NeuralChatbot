...
let StandardBotEngine = function (obj) {
    ...
    if (!obj) {
        ...
        if (!obj.files) {
            logAndThrowUndefined('Action', obj.files[i], 'does not have filterInput function')
        }
        ...
        for (let i = 0; i < obj.files.length; i++) {
            ...
            if (typeof val.computeResult == 'function') {
                logAndThrowUndefined('Action', obj.files[i], 'does not have computeResult function')
            }
