...
let databaseContents = function (obj, userData) {
    ...
    Helper.hasProperties(obj, ['replyTemplate', 'wildcards',
        'results', 'columnMap', 'columnType',
        'columnSynVector', 'confidence'])
    ...
    for (let id = 0; id < val.length; id++) {
        ...
        if (columnProperty && (columnProperty.length > 0)) {
            ...
            if (actualCount == 0) {
                ...
                let slot = slotFiller.reconstructPhrase(replyTemplate.continue[0], wcCopy)
                slotScore = slot.score
                tSet.push(slot.phrase)
            }
            actualCount++;
...
let standard = function (obj, userData) {
    ...
    debug('columnMap', columnMap, 'columnType', columnType, 'columnSynVector', columnSynVector, 'rename', reName)
    //process.exit(0)
    //Make sure these are defined
    Helper.hasProperties(obj, ['replyTemplate', 'wildcards',
        'results', 'columnMap', 'columnType',
    ...
    for (let id = 0; id < val.length; id++) {
        ...
        debug('leadingMatch', leadingMatch, 'columnId', columnId, 'final', Helper.computeElement(leadingMatch, columnId))

        //Fill in the value
        initWc.value = Helper.getObjElement(source, Helper.computeElement(leadingMatch, columnId))
        ...
        if (id == 0) {//The first result can include 'Yes we have....''
        ...
        } else {//Use continuation if there is more than one result
            let slot = slotFiller.reconstructPhrase(replyTemplate.continue[0], initWc);
    ...
    if (((complete == '') || complete.match(/undefined/i)) && replyTemplate.negative[0]) {
        //console.log('yo')
        let tAns = slotFiller.reconstructPhrase(replyTemplate.negative[0], wildcards)
        slotScore = 1;
