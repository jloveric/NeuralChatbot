...
class CreateDatabaseConfig {
        ...
        results.forEach(result => {
          ...
          for (let i of syn) {
            ...
            if (res) {
              if (res != word) {
                Logger.warn(
                  "Synonyms already has entry for",
                  i,
                  res,
                  "replacing with",
                  word
  ...
  initialize(obj) {
    ...
    if (Array.isArray(obj.keywords)) {
      Helper.logAndThrow(
        "CreateDatabaseConfig keywords is an Array and should be an object"
      );
    }
    ...
    for (let i = 0; i < this.config.mapping.length; i++) {
      if (this.config.mapping[i] != "none") {
        this.mappingConfig.addMapping(
          this.config.columns[i],
          this.config.mapping[i]
        );

        //Also, add synonyms
        pList.push(
          this.addSynonyms(this.config.keywords[this.config.columns[i]])
        );
      }
    }
    ...
    let pFinal = new Promise((resolve, reject) => {
      Promise.all(pList)
        ...
        .catch(reason => {
          Logger.error(
            "CreateDatabaseConfig initialize failed for reason",
            reason
          );
          reject(reason);
        });
