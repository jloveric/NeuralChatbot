...
class IndexMongoDatabase {
    ...
    initialize() {
        ...
        let np = this.mongod.initialize(this.phrasedb, this.gf.mongodb.url).then((db) => {
            this.db = db;
            Object.seal(this)
    ...
    deleteElasticsearchIndex(tIndex) {
        ...
        let prom = new Promise((resolve, reject) => {
            this.client.indices.delete({
                ...
                if (err) {
                    Logger.warn('Delete', tIndex, 'elasticsearch error - most likely index does not exist')
                }
                resolve();
            })
        })
    ...
    getDocumentCount(tIndex) {
        let np = new Promise((resolve, reject) => {
            this.client.count({ index: tIndex }).then((resp) => {
    ...
    tQ(tIndex, a, index) {
        ...
        if (index < a.length) {
            ...
            return this.subSet(tIndex, a, index).then(() => {
    ...
    indexTableMongo(tIndex) {
        ...
        let p = new Promise((res, rej) => {
            ...
            let np = new Promise((resolve, reject) => {
                this.db.collection(this.phraseTable).find().toArray((err, item) => {
        ...
        let np = new Promise((resolve, reject) => {
            ...
            prom.then(() => {
                this.client.indices.create({
                    ...
                    this.indexTable(this.indexName, jsonObject).then((count) => {
                        resolve(count)
                    }).catch((reason) => {
                        reject(reason)
                    })
