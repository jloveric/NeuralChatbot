...
class BotInformation {
			...
			this.db.collection(this.botCollection).findOne(command, (err, item) => {
				if (err) {
					...
					if (item) {
						...
						Logger.info("getDocument: No", this.botDatabase, "document for", command, "Ok to upsert")
						resolve(false);
					}
				}
			})
	...
	update(username, body, force) {
		...
		let np = new Promise((resolve, reject) => {
			...
			this.checkIfExists({ user: { $ne: username }, "description.name": body.description.name }).then((ans) => {

				//If it doesn't exist then do an upsert'
				if (!ans) {
					this.db.collection(this.botCollection).updateOne(
						{ user: username },
						{ $set: body },
						{ multi: false, upsert: true, w: 1 }, (err, doc) => {
							if (err) {
	...
	getDocument(username) {
		...
		let np = new Promise((resolve, reject) => {
			this.db.collection(this.botCollection).findOne({ user: username }, (err, item) => {
				if (err) {
					Logger.error("getDocument: findOne error", err);
					reject();
				} else {
					if (item) {
						...
						Logger.error("getDocument: No", this.botDatabase, "document for", username)
						reject();
					}
				}
			})
