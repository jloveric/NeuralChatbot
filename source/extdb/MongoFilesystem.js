...
class MongoFilesystem {
    ...
    getWriteFileStream(filename, user, fileType) {
        ...
        let writeStream = this.bucket.openUploadStream(
            filename, { metadata: { user: user, fileType: fileType } });
    ...
    getReadFileStream(filename, user, fileType) {
        ...
        let np = new Promise((resolve, reject) => {
            ...
            let filter = { filename: filename, metadata: { user: user, fileType: fileType } }
    ...
    getFileAsText(filename, user, fileType) {
        ...
        let np = new Promise((resolve, reject) => {
            p.then((id) => {
                ...
                let chunksQuery = this.db.collection(CHUNKS_COLLECTION).find({ files_id: id });
    ...
    replaceTextInMongo(text, originalName, user, fileType) {
        ...
        let np = new Promise((resolve, reject) => {
            this.deleteAllFiles(originalName, user, fileType).then(() => {
                this.storeTextInMongo(text, originalName, user, fileType).then(() => {
                    resolve();
                }).catch((reason) => {
                    reject();
                })
