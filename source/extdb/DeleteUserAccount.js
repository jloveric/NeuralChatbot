...
class DeleteUserAccount {
	...
	deleteAccount(conf) {
		...
		if (!Helper.hasProperties(conf, ['fileDatabase', 'messageDb', 'usernameDb', 'user', 'mongoUrl','botDatabase'])) {
		...
		let np1 = new Promise((resolve, reject) => {
			p1.then(() => {
				mongod.deleteUserAccount(user).then(() => {
		...
		let np2 = new Promise((resolve, reject) => {
			p2.then((db) => {
				db.collection(user).drop(
					(err, response) => {
						if (err) {//Like the db doesn't exist
							Logger.warn('Error dropping messaging database for user', user, 'since database does not exist');
							resolve();
						} else {
		...
		let np3 = new Promise((resolve, reject) => {
			p3.then((db) => {
				db.collection('accounts').deleteMany({ username: user },
					(err, res) => {
						if (err) {
							Logger.error(err);
							resolve();
		...
		let np4 = new Promise((resolve, reject)=>{
			p4.then((db)=>{
				db.collection('bots').deleteMany({ user: user },
					(err, res) => {
						if (err) {
							Logger.error(err);
							resolve();
		...
		let final = new Promise((resolve, reject) => {
			Promise.all([np1, np2, np3, np4]).then(() => {
