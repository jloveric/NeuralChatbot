...
class StartupDBSearch {
	...
	initializeLogstashFromStdIn(filename, databaseName) {
		...
		this.logstashFinished = new Promise((resolve, reject) => {
			...
			Logger.info('Starting logging engine with file', filename, 'from stdin database', databaseName);
			if (!databaseName) {
				Logger.error('Database name must be specified for static logstash start, currently undefined.')
			}

			this.logstash = exec('dependencies/logstash/bin/logstash -f ' + filename + ' < ' + databaseName,
    ...
    debug('New options', options)
		...
		debug('Called initializeLogstashFromMongoStdIn', databaseName, filename, user)
		...
		this.logstashFinished = new Promise((resolve, reject) => {
			...
			this.esOp.hasIndex(indexName).then((ans) => {
				...
				return mg.initialize(databaseName).then(() => {
					...
					this.logstash = spawn('dependencies/logstash/bin/logstash',
						['-e', configString],
	...
	closeElasticsearch() {
		...
		let np = new Promise((resolve, reject) => {
			if (this.elasticSearch) {
				...
				kill.on('error', function (err) {
					Logger.error('Failed to kill elasticsearch: may have already been killed.')
				});
				kill.on('close', function (code, signal) {
					resolve(code);
				})
