...
class CreateLogstashConfig {
    ...
    initialize(lsConfig) {
        ...
        if(!Helper.hasProperties(lsConfig,
                ['csvFilename','fields','separator','useStdIn','fileDatabase','user'])){
        ...
        this.filter = 'filter {\n\t csv { \n\t\tcolumns => [' + fields + ']\n\t\tseparator=> "' + separator + '"\n\t}\n}\n\n';
        this.output = 'output {\n\t elasticsearch {\n\t\t hosts => ["'+this.gf.elasticsearch.host+'"] \n\t\t document_id => "%{'+Helper.extraId+'}" \n\t\t index => "' + filename + '"\n\t}\n}\n\n';
    ...
    cleanFields(fields) {
        for (let i = 0; i < fields.length; i++) {//Check if there is a quote
    ...
    writeToMongo(filename, user) {
        ...
        let np = this.MongoFilesystem.replaceTextInMongo(
			this.getConfig(), filename, user, "logstash");
