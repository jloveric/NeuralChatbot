...
class InstallBot {
    ...
    initialize() {
        ...
        let p1 = botDb.initialize('botDatabase', this.gc.mongodb.url).then((thatDb) => {
            this.bDb = thatDb;
    ...
    createBotInfo(username, info) {
        ...
        return bi.initialize(this.gc.mongodb.botDatabase, this.gc.mongodb.url, 'bots').then(() => {
            ...
            let np = new Promise((resolve, reject) => {
                ...
                this.bDb.collection('bots').findOne({ user: username }, (err, item) => {
                    if (err) {
    ...
    installDatabase(path, csvFile, user) {
        ...
        return mfs.initialize(dbName).then(() => {
            return mfs.replaceFileInMongo(
                path, csvFile,
                user, "databaseTemp")
        }).then(() => {
    ...
    delAndRegisterUser(username, password, info) {
        ...
        return delAccount.deleteAccount({
        ...
        let np = new Promise((resolve, reject) => {
            this.initialize().then(() => {
                ...
                mfs.initialize(dbName).then(() => {
                    ...
                    let fileAsText = null;
                    let tableName = null;
                    mfs.getDocumentsOfType(username, 'phrase').then((files)=>{
                        ...
                        return BasicPhrasexDatabase.generatePhraseDatabase(null, tableName, fileAsText);
                    }).then(()=>{
                        ...
                        Logger.error('Failed to install local phrase database for reason', reason)
                        resolve(); 
                    })
        ...
        let np = new Promise((resolve, reject) => {
            this.initialize().then(() => {
                ...
                jsonfile.readFile(filename, (err, obj) => {
                    ...
                    debug('Before del and register')
                    this.delAndRegisterUser(username, password, obj).then(() => {
