	...
	checkConsistency(config) {
		...
		let pBot =
			p1.then((db) => {
				...
				return new Promise((resolve, reject) => {
					this.db.collection('bots').findOne({ "user": config.user }, (err, item) => {
						if (err) {
							reject(err);
						} else if (item) {
		...
		let pMongo =
			Promise.all([p2, pBot]).then((ans) => {
				...
				if (botType != "mongo") {
					pList.push(this.mongoFilesystem.doesFileExist(filename, config.user, 'database'))
					pList.push(this.mongoFilesystem.doesFileExist(filename, config.user, 'logstash'))
					pList.push(this.mongoFilesystem.doesFileExist(filename, config.user, 'databaseConfig'))
