...
describe("ProcessDatabase", function () {
    ...
    it("Should process the database", function (done) {
        ...
        let prom = mongoFile.initialize(fileDB).then(() => {
            let p1 = mongoFile.deleteFile('groceries.csv', 'john.loverich@gmail.com', 'databaseTemp')
            let p2 = mongoFile.deleteFile('groceries.csv', 'john.loverich@gmail.com', 'database')
            return Promise.all([p1, p2])
        }).then(() => {
            return mongoFile.storeFileInMongo('uploads/groceries.csv',
                'groceries.csv',
                'john.loverich@gmail.com',
                'databaseTemp')
        }).then(() => {
            console.log('finished mongo work')
            return dbModify.initialize('groceries.csv',
                'john.loverich@gmail.com',
                'databaseTemp',
                fileDB)
        }).then(() => {
            ...
            let np = pd.initialize('groceries.csv', 'john.loverich@gmail.com', fileDB);
