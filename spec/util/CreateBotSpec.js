...
describe("Testing the utility CreateBot", function() {
  it("Should create a new bot", function(done) {
    let np = new Promise((resolve, reject) => {
      exec(
        "source/util/CreateBot.js --username random --password tunafish --install uploads/cbottest.install",
        (error, stdout, stderr) => {
          if (error) {
  ...
  it("Should fail since bot exists", function(done) {
    ...
    exec(
      "source/util/CreateBot.js --username blubber --password tunafish --install uploads/cbottest.install",
      (error, stdout, stderr) => {
        if (error) {
          expect(false).toEqual(true);
        }
  ...
  it("Should fail since username is missing", function(done) {
    ...
    exec(
      "source/util/CreateBot.js --password tunafish --install uploads/cbottest.install",
      (error, stdout, stderr) => {
        if (error) {
          expect(false).toEqual(true);
        }
