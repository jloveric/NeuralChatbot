...
describe("Testing Phrasex", function () {
    ...
    it("Should make guesses with ambiguous data", function (done) {
        ...
        pList.push(simpleTest(phrasex, 
            "the tacos are in aisle 2", { item: 'tacos', column : "aisle", value : "2" }))
    ...
    it("Should produce reasonable scores", function (done) {
        ...
        phrasex.getWildcardsAndMatch("May I speak with hi my name is jake", [], userData).then((ans) => {
            console.log(ans)
            expect(ans[0].confidence).toBe(1)
            return phrasex.getWildcardsAndMatch("May I speak with jake", [], userData)
        }).then((ans) => {
            console.log(ans)
            expect(ans[0].confidence).toBeTruthy(1)
            return phrasex.getWildcardsAndMatch("I may speak with jake", [], userData)
        }).then((ans) => {
            console.log(ans)
